...
def storing(zone, network_file, Tij, k):
    ...
    for i in range(len(edges)):
        ksp = find_ksp_routes(k, zone, network_file)
        copy_ksp = find_ksp_routes(k, zone, network_file)
        mod_ksp = assign_vehs_to_routes(Tij, ksp, network_file, copy_ksp)
        edge_id = edges[i]
        df = find_pij(mod_ksp, edge_id)
        df1 = df.values.tolist()
        df2 = flatten(df1)
        j_s = ",".join([str(i) for i in df2])
        l.append(tuple((edge_id, j_s)))
        """ Stored data using this code """
        # sqlite_file = 'Pijs.db'
        # conn = sqlite3.connect(sqlite_file)
        # curr = conn.cursor()
        # curr.execute('''CREATE TABLE edges_3
        #               (edge text, pij real)''')

        # curr.executemany('INSERT INTO edges_3 VALUES(?,?);',l)
        # conn.commit()
        # conn.close()
        return sqlite_file

def retrieve(sqlite_file, edgeid):

        conn = sqlite3.connect(sqlite_file)
        curr = conn.cursor()

        curr.execute('SELECT * FROM edges_3')
        s = 0
        p = []
        for row in curr:
            for i in row:
        ...
        re = []
        n = len(p)/len(zone)
        sp = np.array_split(p, n)
        for array in sp:
            re.append(list(array))

        column = (list(zone.keys()))
        dfs = pd.DataFrame(re, index=column)
        dfs.columns = column
        return dfs
